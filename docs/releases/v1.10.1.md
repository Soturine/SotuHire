...
- `SHA256SUMS`;
- `sotuhire-sbom-v1.10.1.cdx.json` (CycloneDX 1.5).

## Validação final

- extensão e SBOM gerados; checksums SHA-256 verificados;
- `pytest -m external_ai`: 2 aprovados e 2 bloqueados por quota OpenAI;
- provider-diagnostic: local/Gemini válidos e OpenAI classificado como bloqueio externo;
- provider-structured-output e release-smoke: concluídos sem regressões ocultas;
- matriz completa local não foi repetida por solicitação do mantenedor; CI, CodeQL e Docs remotos são obrigatórios e precisam estar verdes antes da tag.

Os arquivos publicados acompanham `SHA256SUMS`; nenhum resultado bloqueado por quota é descrito como aprovação de provider.
