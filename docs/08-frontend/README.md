...
O fluxo `AUTHENTICATED_BROWSER` existente no backend local tambem aparece nessa tela. A v1.8.0 nao
...
GitHub Analysis ou Candidaturas. A v1.8.0 tambem permite revisar, arquivar ou ignorar capturas no
historico local.
...
CSV/JSON recorrente, links manuais e fontes observadas. Feeds RSS/Atom publicos agora podem ser
usados manualmente pelo Radar de Vagas; APIs oficiais continuam como adapters planejados.

## Radar de Vagas

A rota `/radar` adiciona o fluxo v1.8.0:

- wishlist local;
- fontes RSS/Atom publicas;
- rodadas manuais;
- resultados com score, evidencias e lacunas;
- alertas locais;
...
O Playwright cobre fluxo guiado, demos de analise, IA Settings, Fontes e Captura, Radar, Kanban,
cross-browser e ausencia de branding legado. O spec visual gera a serie `sotuhire-v1.8-web-*.png`
