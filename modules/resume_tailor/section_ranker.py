...
def rank_resume_sections(job_text: str, available_sections: list[str]) -> list[str]:
    ...
    for section in available_sections:
        ...
        if "project" in key or "projeto" in key:
            scores[section] += (
                3 if any(term in text for term in ["github", "projeto", "portfolio"]) else 1
            )
        if "skill" in key or "compet" in key:
            scores[section] += (
                2 if any(term in text for term in ["python", "sql", "java", "qa"]) else 1
            )
        if "educ" in key or "form" in key:
            scores[section] += (
                2 if any(term in text for term in ["estágio", "trainee", "engenharia"]) else 1
            )
