...
def test_popup_opens_resume_studio_with_identifiers_only() -> None:
    popup = Path("browser-extension/popup.js").read_text(encoding="utf-8")
    html = Path("browser-extension/popup.html").read_text(encoding="utf-8")

    assert 'data-action="resume-studio"' in html
    flow = popup.split("async function openResumeStudio()", maxsplit=1)[1].split(
        "const act =", maxsplit=1
    )[0]
    assert 'sendOrQueue("/capture/job", capture' in flow
    assert 'new URL("/resume-studio"' in flow
    assert 'target.searchParams.set("capture_id", saved.capture_id)' in flow
    assert 'target.searchParams.set("job_snapshot_id", saved.snapshot_id)' in flow
    assert "visible_text" not in flow.split("const target =", maxsplit=1)[1]


def test_extension_sends_deterministic_idempotency_header() -> None:
    popup = Path("browser-extension/popup.js").read_text(encoding="utf-8")
    background = Path("browser-extension/background.js").read_text(encoding="utf-8")

    assert "queueRuntime.idempotencyKey(path, body)" in popup
    assert '"X-SotuHire-Idempotency-Key"' in background
