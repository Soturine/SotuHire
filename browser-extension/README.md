...
Extensão Manifest V3 para capturar a página atual, uma lista visível de candidaturas ou um projeto público e enviar os dados para a Local Companion API em `http://127.0.0.1:8765`.

O app SotuHire v1.9.2 é compatível com a extensão v0.9.0. A extensão tem versionamento independente.
...
Depois abra **Fontes e Captura > Extensão Local** para ver capturas salvas, revisar histórico, arquivar itens locais, importar para Vaga/GitHub/Candidaturas e gerar evidências candidatas para o Perfil Profissional Universal.
...
A chave Gemini standalone opcional pode permanecer no popup, mas ela é separada da IA configurada no app. A API key do app SotuHire não é lida, exposta ou armazenada pela extensão.
