"""Capture only the fictional v1.11.0 security, local-AI and intelligence surfaces."""
...
GIF_TARGET = OUTPUT_DIR / "sotuhire-v1.11.0-security-local-ai-intelligence.gif"
...
def capture(base_url: str) -> None:
    ...
    with sync_playwright() as playwright:
        ...
        _shot(page, base_url, "/settings", "sotuhire-v1.11.0-settings-categories.png", frames)
        local_ai = page.get_by_text("IA local e compatível", exact=True).locator(
            "xpath=ancestor::section[1]"
        )
        local_ai.scroll_into_view_if_needed()
        page.wait_for_timeout(200)
        _save_locator(local_ai, "sotuhire-v1.11.0-local-ai-health.png", frames)
        _shot(page, base_url, "/intelligence", "sotuhire-v1.11.0-career-analytics.png", frames)
        _shot(page, base_url, "/privacy", "sotuhire-v1.11.0-security-status.png", frames)
        browser.close()
    _gif(frames)
...
def _save_locator(locator: Locator, name: str, frames: list[Path]) -> None:
    target = OUTPUT_DIR / name
